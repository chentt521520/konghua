...
public class safepho extends BaseActivity_konghua implements verifyidentity.verifyidentitysuc {
    ...
    public static safephointerface safe;

    @Override
    protected void AddView() {
        businessDh.settext_("修改安全手机");
    ...
    protected int getLayout() {
        return R.layout.activity_safepho;
    }
        ...
        switch (view.getId()) {
            case R.id.paypwd_timerte:
                if (TextUtils.isEmpty(paypwdEdyzm.getText().toString()) || paypwdEdyzm.getText().toString().length() < 11) {
                    Toast.makeText(safepho.this, "请输入11位手机号", Toast.LENGTH_SHORT).show();
            ...
            case R.id.ywpho_teok:
                ...
                if (ywphoEdmmre.getText().toString().length() < 6) {
                    Toast.makeText(safepho.this, "请输入6位验证码", Toast.LENGTH_SHORT).show();
                    TimerTextView.isc = false;
                    return;
                }
                ...
                try {
                    ...
                    try {
                        ...
                        if (jsonObject.getString("status").equals("1")) {
                            Toast.makeText(safepho.this, "更换成功", Toast.LENGTH_SHORT).show();
                            if (safe != null)
                                safe.safephosuc();
                            finish();
                            ...
                            if ((msgx.equals("validate_code_is_error"))) {
                                Toast.makeText(safepho.this, "验证码错误", Toast.LENGTH_SHORT).show();
                            } else if ((msgx.equals("validate_code_is_null"))) {
                                Bundle bundle = new Bundle();
                                bundle.putInt("type", 3);
                                bundle.putString("pho", getIntent().getExtras().getString("pho"));
                                startActivityByIntent(safepho.this, verifyidentity.class, bundle);
                            } else
                                Toast.makeText(safepho.this, jsonObject.getString("msg"), Toast.LENGTH_SHORT).show();
                        }
            ...
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                ...
                try {
                    ...
                    try {
                        JSONObject jsonObject = new JSONObject(str);
                        if (jsonObject.getString("status").equals("1")) {
                            Toast.makeText(safepho.this, "短信发送成功", Toast.LENGTH_SHORT).show();
                        } else {
                            Toast.makeText(safepho.this, jsonObject.getString("msg"), Toast.LENGTH_SHORT).show();
                        }
                    } catch (JSONException e) {
    ...
    public interface safephointerface {
        void safephosuc();
    }

    public static void setSafe(safephointerface safe) {
        safepho.safe = safe;
    }
